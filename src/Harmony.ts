...
export class Harmony {
  ...
  static isPitchClass(note) {
    return Note.pc(note) === note;
  }

  static hasSamePitch(noteA: string, noteB: string, ignoreOctave = false) {
    if (ignoreOctave || Harmony.isPitchClass(noteA) || Harmony.isPitchClass(noteB)) {
  ...
  static transposeChord(chord: string, interval) {
    if (!chord) {
      return chord;
    }
    const tokens = Chord.tokenize(Harmony.getTonalChord(chord));
    let root = <string>Distance.transpose(tokens[0], interval);
  ...
  static getPitchesInRange(pitches: string[], range: string[]) {
    return noteArray(range).filter(n => pitches.find(p => Note.chroma(p) === Note.chroma(n)));
